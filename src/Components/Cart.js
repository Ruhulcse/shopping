...
const Cart = () => {
  ...
  //counting idential item with parcentage
  let n = (cartItems.length-uniqueItems.length)*10;
  //counting discount
  const total = Object.values(cartItems).reduce((t, {price}) => t + parseInt(price), 0)
  ...
  const makeOrder = async (e) =>{
        ...
        try {
          const {data} = await axios.post('https://fakestoreapi.com/products',cartItems);
          if(data){
    ...
    <>
      {showCart && (
        <div className='cart__wrapper'>
          ...
          <div>{!child&&<button onClick={makeOrder} className="bt">order</button>}</div>
        </div>
      )}
    </>
  );
